...
import { collection, addDoc, getDocs, deleteDoc, doc, Timestamp, query, orderBy, updateDoc, serverTimestamp, setDoc } from 'firebase/firestore';
...
export default function WordGroupsManagementPage() {
  ...
  // Unsplash API'den görsel çekme fonksiyonu
  const fetchImageFromUnsplash = async (word: string) => {
    setIsLoadingImage(true);
    setImageError('');
    try {
      const response = await fetch(
        `https://api.unsplash.com/search/photos?query=${encodeURIComponent(word)}&client_id=YOUR_UNSPLASH_API_KEY&per_page=5`,
        {
          headers: {
            'Accept-Version': 'v1',
      ...
      if (!response.ok) {
        setImageError('API limiti veya hata!');
        return;
      }
      const data = await response.json();
      if (data.results && data.results.length > 0) {
        const newImages = data.results.map((img: any) => ({ id: img.id, url: img.urls.regular }));
        setImages(newImages);
        setSelectedImageUrl(newImages[0].url);
        setNewWord(w => ({...w, imageUrl: newImages[0].url}));
      } else {
  ...
  const handleAddWord = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setImages([]);
      setSelectedImageUrl('');
      setTimeout(() => setShowAddWordModal(false), 1200);
      fetchWordGroups();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end mb-4">
        <Link href="/dashboard/content/word-groups/add-word">
          <button
      ...
      {showAddWordModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg w-full max-w-lg overflow-hidden">
            <div className="px-6 py-4 bg-blue-50 border-b border-blue-100 flex justify-between items-center">
              <h3 className="text-lg font-semibold text-blue-800">Yeni Kelime Ekle</h3>
              <button onClick={() => setShowAddWordModal(false)} className="text-gray-500 hover:text-gray-700 focus:outline-none">
                <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ...
            <form onSubmit={handleAddWord} className="p-6 space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => fetchImageFromUnsplash(newWord.english)} className="px-3 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500" disabled={isLoadingImage || !newWord.english.trim()}><Image size={20} /></button>
                </div>
              </div>
              <div>
              ...
              <div className="mt-6 flex justify-end space-x-3">
                <button type="button" onClick={() => setShowAddWordModal(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">İptal</button>
